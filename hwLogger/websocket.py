        ...
        async with websockets.connect(
            ...
            try:
                ...
                if auth_msg.get("type") != "authorization_requested":
                    logger.warning(f"Expected authorization_requested, got: {auth_msg.get('type')}")
                    
            except asyncio.TimeoutError:
                logger.error("Timeout waiting for authorization_requested message")
                raise
            except Exception as e:
            ...
            try:
                auth_response = json.dumps({"type": "authorization", "data": self.token})
            ...
            try:
                ...
                if confirm_msg.get("type") == "authorized":
                    ...
                    raise Exception(f"Authorization failed: {confirm_msg.get('data')}")
                    ...
                    try:
                        ...
                        if "data" in data:
                            if isinstance(data["data"], dict):
                                ...
                                logger.debug(f"Received [{msg_type}]: {keys}...")
                            else:
                                logger.debug(f"Received [{msg_type}]: {str(data['data'])[:80]}")
                        else:
                            logger.debug(f"Received [{msg_type}]")
                        
                        # Log full measurement data for detailed inspection
                        if msg_type == "measurement":
                            logger.debug(f"Full measurement: {json.dumps(data.get('data', {}), indent=2)}")
