    ...
    async def _handle_ws_data(self, data: dict):
        ...
        try:
            ...
            if self._last_30s_time is None:
                ...
                logger.debug(
                    f"30s block {current_30s_time.isoformat()}: "
            ...
            current_boundary_time = now.replace(
                minute=current_boundary, second=0, microsecond=0
            )

            if self._last_log_time is None:
                self._last_log_time = current_boundary_time
            elif (
                now >= current_boundary_time
                and self._last_log_time < current_boundary_time
            ):
                ...
                logger.info(
                    f"5-min block {current_boundary_time.isoformat()}: "
